...
global $conn;
$searchKey="where 1";
if($_REQUEST['key']) $searchKey.=" and (temp.username like '%{$_REQUEST['key']}%' or temp.provider like '%{$_REQUEST['key']}%' or temp.email like '%{$_REQUEST['key']}%')";
//fetch
$sql="select temp.* from(select users.id as uid,users.email as email,users.username as username,users.contact_number as mobile,users.paypal_email as paypal,(select group_concat(social_provider.provider separator '<br>') from social_provider where social_provider.user_id=users.id) as provider,(SELECT 0.5*(0.7*(sum(pricing.price))) from pricing join budget on budget.price_id=pricing.id join social_provider on social_provider.hash_id=budget.hash_id and MONTH(CURDATE())=MONTH(social_provider.created_on) where social_provider.`user_id` = users.id) as p_month, payout.amount_earned as earned,payout.amount_paid as paid from users left join payout on users.id=payout.user_id where users.role='Member') as temp $searchKey ";
											...
											<label style="width:100%;">
												...
												<div class="input-group">
													<input class="form-control" type="text" aria-controls="example1" 
														onkeyup="if(event.keyCode==13){ window.location.href='?limit=<?php echo $limit; ?>&status=<?php echo $_GET['status']; ?>&key='+this.value; }" placeholder="Search by Username/Provider/Email" value="<?php echo $_GET['key']; ?>">
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Amount Earned<br>(All Time)</th>
                                                 <th>Amount Earned<br>(This Month)</th>
                                                <th>Amount Paid</th>
                                                <th> </th>
